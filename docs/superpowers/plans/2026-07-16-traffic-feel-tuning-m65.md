...
- Guard bands that must hold at every task: `signal.startup_lost_s` ∈ [1, 4]; `yield4.completed` ≥ 13; `grid.delay_index` never above its incoming value +5%; perf ceilings unchanged (validate500 < 150 ms, tick300 < 8 ms).
